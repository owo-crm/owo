...
const leadToneClasses: Record<LandingConfig["hero"]["leads"][number]["tone"], string> = {
    ...
    <section id="top" className="px-4 pt-28 sm:px-6 sm:pt-32">
      <div className="mx-auto w-full max-w-6xl">
        <div className="grid items-center gap-10 lg:grid-cols-2">
          ...
          >
            <p className="mb-4 text-sm text-[#8b8d9e]">{config.tagline}</p>
            <p className="mb-4 inline-flex items-center gap-2 rounded-full border border-[#6b7ff0]/35 bg-[#6b7ff0]/12 px-3 py-1 text-xs text-[#aeb9ff]">
              {config.hero.prelaunchBadge}
            </p>
          ...
          >
            ...
            <div className="mt-4 space-y-3">
              {config.hero.leads.map((lead, index) => (
                <div
        ...
        <p className="mt-8 text-center text-sm text-[#8b8d9e]">{config.hero.trustLine}</p>
      </div>
    </section>
  );
}
