...
export default function DigitalSerenity() {
  ...
  useEffect(() => {
    ...
    const handleClick = (event: MouseEvent) => {
      const newRipple: Ripple = { id: Date.now() + Math.floor(Math.random() * 1000), x: event.clientX, y: event.clientY };
  ...
  useEffect(() => {
    const wordElements = document.querySelectorAll<HTMLSpanElement>(".word-animate");
    ...
    const handleScroll = () => {
      if (!scrolled) {
        ...
        floatingElementsRef.current.forEach((el, index) => {
          const delay = (Number.parseFloat(el.style.animationDelay || "0") || 0) * 1000 + index * 100;
          const timer = window.setTimeout(() => {
  ...
  return (
    <>
      ...
      <div className="relative min-h-screen overflow-hidden bg-gradient-to-br from-slate-900 via-black to-slate-800 font-primary text-slate-100">
        ...
        <div className="relative z-10 flex min-h-screen flex-col items-center justify-between px-6 py-10 sm:px-8 sm:py-12 md:px-16 md:py-20">
          ...
          <div className="relative mx-auto max-w-5xl text-center">
            ...
            <div
              className="absolute -left-6 top-1/2 h-px w-3 -translate-y-1/2 transform bg-slate-300 opacity-0 sm:-left-8 sm:w-4"
              style={{ animation: "word-appear 1s ease-out forwards", animationDelay: "3.2s" }}
            />
            <div
              className="absolute -right-6 top-1/2 h-px w-3 -translate-y-1/2 transform bg-slate-300 opacity-0 sm:-right-8 sm:w-4"
